...
-- Place this LocalScript in StarterPlayerScripts (or upload raw to GitHub and use loadstring)
-- Compact UI, avatar toggle button (circular), tabs: Players | Menu | Speed
-- Features: Teleport, ESP (name+distance+tracer, auto-update), Infinite Jump, Noclip (fixed), Touch Fling (500), Anti Fall Damage, Ghost Mode (A), TP Aura (nearest)
-- Mobile-friendly, draggable toggle & panel, menu scroll when full
...
knob.InputBegan:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
        draggingKnob = true
        i.Changed:Connect(function() if i.UserInputState == Enum.UserInputState.End then draggingKnob = false end end)
    end
end)
knob.InputChanged:Connect(function(i)
    if draggingKnob and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
        local abs = i.Position
        local bgPos = sliderBg.AbsolutePosition
        local relX = abs.X - bgPos.X
        setSpeed(relX / math.max(1, sliderBg.AbsoluteSize.X))
    end
end)
...
Players.PlayerAdded:Connect(function(plr)
    plr.CharacterAdded:Connect(function() task.wait(0.2) if espOn and plr ~= LocalPlayer then makeESP(plr) end end)
end)
...
LocalPlayer.CharacterAdded:Connect(function(c) task.wait(0.25) if noclipOn then applyNoclip(true) end end)

-- ===== Touch Fling (500 default) =====
local flingOn = false
    ...
    if pl and pl ~= LocalPlayer then
        ...
        if hrp then
            pcall(function()
                ...
                local sourcePos = (LocalPlayer.Character and LocalPlayer.Character.PrimaryPart and LocalPlayer.Character.PrimaryPart.Position) or hrp.Position
...
local function bindTouchesToChar(char)
    unbindTouches()
    for _,p in ipairs(char:GetDescendants()) do
        if p:IsA("BasePart") then
            table.insert(touchConns, p.Touched:Connect(onTouched))
        end
    end
end
function unbindTouches()
