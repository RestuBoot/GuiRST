...
Subtitle.Text = "Premium Script Access"
...
Rules.Text = "This script is exclusive get the key to unlock full access!"
...
-- FUNGSI MEMBUAT ESP UNTUK SATU PEMAIN
local function CreateESPForPlayer(player)
    ...
    -- LINE ESP (dari root ke root)
    local line = Instance.new("Attachment")
    line.Name = "ESP_Line"
    line.Parent = root

    local line2 = Instance.new("Attachment")
    line2.Name = "ESP_LineTarget"
    line2.Parent = root

    local lineObj = Instance.new("LineHandleAdornment")
    lineObj.Name = "ESP_LineHandle"
    lineObj.Attachment0 = line
    lineObj.Attachment1 = line2
    lineObj.Length = 1
    lineObj.Color3 = Color3.fromRGB(0, 255, 255)
    lineObj.Transparency = 0.3
    lineObj.Thickness = 1
    lineObj.ZIndex = 10
    lineObj.AlwaysOnTop = true
    lineObj.Parent = root
    table.insert(objects, lineObj)

    -- TRACER (dari kepala ke kepala)
    local tracer = Instance.new("Attachment")
    tracer.Name = "ESP_Tracer"
    tracer.Parent = head

    local tracer2 = Instance.new("Attachment")
    tracer2.Name = "ESP_TracerTarget"
    tracer2.Parent = head

    local tracerObj = Instance.new("LineHandleAdornment")
    tracerObj.Name = "ESP_TracerHandle"
    tracerObj.Attachment0 = tracer
    tracerObj.Attachment1 = tracer2
    tracerObj.Length = 1
    tracerObj.Color3 = Color3.fromRGB(255, 0, 0)
    tracerObj.Transparency = 0.4
    tracerObj.Thickness = 1
    tracerObj.ZIndex = 10
    tracerObj.AlwaysOnTop = true
    tracerObj.Parent = head
    table.insert(objects, tracerObj)

    -- CHAMS (karakter berwarna cerah)
    for _, part in pairs(char:GetChildren()) do
        if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" then
            ...
            table.insert(objects, cham)
        end
    end

    -- X-RAY (transparan menembus dinding)
    for _, part in pairs(char:GetChildren()) do
        if part:IsA("BasePart") then
            local xray = Instance.new("Highlight")
            xray.Name = "ESP_XRay"
            xray.Parent = part
            xray.FillColor = Color3.fromRGB(255, 0, 0)
            xray.OutlineColor = Color3.fromRGB(255, 255, 255)
            xray.FillTransparency = 0.7
            xray.OutlineTransparency = 0.3
            xray.AlwaysOnTop = true
            table.insert(objects, xray)
        end
    end

    -- SIMPAN OBJEK ESP UNTUK PEMAIN INI
    ESPObjects[player] = objects
end

-- FUNGSI HAPUS ESP UNTUK SATU PEMAIN
...
-- FUNGSI UPDATE ESP UNTUK SEMUA PEMAIN
...
-- TOGGLE ESP DI TAB VISUAL
CreateToggle(VisualTab,"ESP Complete (Box, Name, Line, Tracer, Chams, X-Ray)",function(v)
    ESPEnabled = v
    if v then
        UpdateESP()
    else
...
-- EVENT SAAT PEMAIN MASUK / KELUAR / RESPAWN / MATI
...
-- UPDATE ESP SETIAP 1 DETIK (untuk respawn / mati)
task.spawn(function()
...
-- NO FOG
CreateToggle(VisualTab,"No Fog",function(v)
    if v then
        Lighting.FogEnd = 100000
...
-- REMOVE CLOUDS
CreateToggle(VisualTab,"Remove Clouds",function(v)
    Lighting.Clouds = not v
end)
